...
export default class Navbar extends Component {
  state = {
    modal: null,
    isUserHavingAskQuestionPermission: false,
  };
  ...
  async componentWillMount() {
    const askQuestionPermission =  await this._hideAskQuestion();
    this.setState({isUserHavingAskQuestionPermission: askQuestionPermission})
  }
  ...
  _hideAskQuestion = async () => {
    const currentUser = await UserApi.current();
    const currentUserGroupId =currentUser.group_ids[1];
    const graph = await PermissionsApi.graph();
    const currentUserPermission = graph.groups[currentUserGroupId][1];
    return !(currentUser.is_superuser === false && currentUserPermission.schemas === 'all');
  }
  renderAdminNav() {
    return (
    ...
    return (
      ...
      >
        ...
        <Flex ml="auto" align="center" className="relative z2">
          { this.state.isUserHavingAskQuestionPermission && (
            <Link
              to={Urls.newQuestion()}
              mx={2}
